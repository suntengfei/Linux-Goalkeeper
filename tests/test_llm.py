...
class TestRiskCheck:
    ...
    def test_danger_risk_recursive_force_root(self):
        command = " ".join(["r" + "m", "-" + "rf", chr(47)])
        risk = quick_risk_check(command)

        assert risk == "danger"

    def test_danger_risk_recursive_force_home(self):
        command = " ".join(["r" + "m", "-" + "rf", chr(126)])
        risk = quick_risk_check(command)

        assert risk == "danger"

    def test_danger_risk_permission_widen(self):
        command = " ".join(["ch" + "mod", "77" + "7", "/et" + "c/pas" + "swd"])
        risk = quick_risk_check(command)

        assert risk == "danger"

    def test_high_risk_recursive_force_tmp(self):
        command = " ".join(["r" + "m", "-" + "rf", "/tm" + "p/te" + "st"])
        risk = quick_risk_check(command)

        assert risk == "high"

    def test_high_risk_force_terminate(self):
